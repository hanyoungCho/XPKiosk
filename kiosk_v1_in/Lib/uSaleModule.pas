...
//    // 매출 및 결제 데이타를 DB에 저장한다.
  ...
  private
    ...
    FAdvertisementListTeeboxUp: TList<TAdvertisement>;
    FAdvertisementListDown: TList<TAdvertisement>;
    FAdvertisementListPopup: TList<TAdvertisement>;
...
//    FingerStr: UTF8String;
    ...
    property AdvertisementListTeeboxUp: TList<TAdvertisement> read FAdvertisementListTeeboxUp write FAdvertisementListTeeboxUp;
    property AdvertisementListDown: TList<TAdvertisement> read FAdvertisementListDown write FAdvertisementListDown;
    property AdvertisementListPopup: TList<TAdvertisement> read FAdvertisementListPopup write FAdvertisementListPopup;
...
begin
  try
    ...
    begin
      ...
      begin
        Log.D('카드제휴 할인 대상 아님 - 할인금액 0', ARecvInfo.CardBinNo);
        Log.D('카드제휴 할인 대상 아님', ACode);
        Log.D('카드제휴 할인 대상 아님', AMsg);
      end;
    end
...
begin
  ...
  AdvertisementListTeeboxUp := TList<TAdvertisement>.Create;
  AdvertisementListDown := TList<TAdvertisement>.Create;
  AdvertisementListPopup := TList<TAdvertisement>.Create;
...
begin
  ...
  try
    ...
    if AdvertisementListTeeboxUp <> nil then
      AdvertisementListTeeboxUp.Free;

    if AdvertisementListDown <> nil then
      AdvertisementListDown.Free;

    if AdvertisementListPopup <> nil then
      AdvertisementListPopup.Free;
...
begin
  try
    try
      ...
      begin
        //MemberList := Global.Database.GetAllMemberInfo

        rMemberInfoList := Global.Database.GetAllMemberInfo;
...
//          if BuyProductList[0].Products.Use or (BuyProductList[0].Products.Product_Div = PRODUCT_TYPE_D) then
            SelectProduct := BuyProductList[0].Products;
      end;

      if SelectProduct.Code <> EmptyStr then
      begin // 예약 배정 등록
  //      if not Global.Database.TeeBoxReservation then
  //      begin
  //        //실패
  //        Log.E('TeeBoxReservation', '예약배정 실패');
  //      end;
...
//      if (BuyProductList[0].Products.Product_Div <> 'D') and (Global.SaleModule.Member.Code <> EmptyStr) then
      begin
        Global.SaleModule.PopUpFullLevel := pflProduct;
