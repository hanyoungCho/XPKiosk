...
//      ADateTime := DateStrToDateTime(FormatDateTime('yyyymmdd', now) + StringReplace(Global.Config.Store.StoreEndTime, ':', '', [rfReplaceAll]) + '00');
      ...
      begin
//        if (Global.Config.Store.StoreCloseStartTime <= FormatDateTime('yyyy-mm-dd hh:nn', now)) and
//        (Global.Config.Store.StoreCloseEndTime >= FormatDateTime('yyyy-mm-dd hh:nn', now)) then
...
begin
  try
    try
      ...
      AEndTime := DateStrToDateTime(FormatDateTime('yyyymmdd', now) + StringReplace(Global.Config.Store.StoreEndTime, ':', '', [rfReplaceAll]) + '00');
      //AEndTime := DateStrToDateTime(FormatDateTime('yyyymmdd', now) + StringReplace('18:30', ':', '', [rfReplaceAll]) + '00');
