...
//       TSelectBoxTopMapItemStyle(ItemList[Index]).Free;
    ...
    begin
      if Global.TeeBox.TeeBoxList[Index].FloorCd = AFloor then
        Inc(AFloorCnt);
    end;
    ...
    begin
      ...
      if TopMarginCnt <= ((Global.TeeBox.FloorMaxTeeboxCnt div 2) + AFloorCnt_Mod) then
      ...
      //chy 캐슬렉스 층마다 타석수가 달라서 가장큰수로 통일
      if Global.TeeBox.FloorMaxTeeboxCnt > FLOOR_MAX_CNT then
      begin
        Text.Visible := False;
