...
//    RemoveObject(AFullPopupCouponItem);
    APosition.Free;
    FreeAndNil(APoint);
  end;
//  ProductList := TList<TProductInfo>.Create;
//
//  ProductList := Global.SaleModule.ProductList;
//
//  X := 0;
//  Y := 0;
//  RowIndex := 0;
...
//  ItemRectangle.DeleteChildren;
//
//  for Index := 0 to ProductList.Count do
//  begin
////    if (Global.SaleModule.memberItemType = mitCoupon) and (ProductList[Index - 1].Product_Div <> 'C') then
////      Continue;
//
//    if Index = 0 then
//      AProductInfo.Code := EmptyStr
//    else
//    begin
//      AProductInfo := ProductList[Index - 1];
//      {
//      if Global.SaleModule.PopUpFullLevel = pflPeriod then
//        if AProductInfo.Product_Div <> 'R' then
//          Continue;
//
//      if Global.SaleModule.PopUpFullLevel = pflCoupon then
//        if AProductInfo.Product_Div <> 'C' then
//          Continue;
//      }
//      if Global.SaleModule.VipTeeBox then
//      begin
//        if AProductInfo.ZoneCode <> 'V' then
//          Continue;
//      end
//      else
//      begin
...
//    AFullPopupCouponItem := TFullPopupCouponItem.Create(nil);
//
//    APosition.X := AFullPopupCouponItem.Width;
//    APosition.Y := (RowIndex * AFullPopupCouponItem.Height);
//
//    AFullPopupCouponItem.Position := APosition;
//
//    ItemRectangle.Height := ItemRectangle.Height + AFullPopupCouponItem.Height;
