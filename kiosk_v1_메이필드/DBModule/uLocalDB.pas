...
type
  TLocalDB = class
    ...
    public
      ...
      function DelMemberListToDB: Boolean; //회원정보 삭제
  end;
...
function TLocalDB.DBConnection: Boolean;
begin
  Result := False;
  try
    try
...
function TLocalDB.GetMemberListToDB: TList<TMemberInfo>;
var
  sSql: String;
  nIndex: Integer;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        rMemberInfo.WelfareCd := FieldByName('welfare_cd').AsString; //복지카드 고유번호		welfare_cd
        //사원 번호		emp_no
        //회원 포인트		member_point
        //회원 할인율		dc_rate
...
function TLocalDB.SetMemberToDB(AMemberInfo: TMemberInfo): Boolean;
var
    ...
    try
      ...
      try
        ...
        sSql := ' INSERT INTO MEMBER ' +
                '( MEMBER_NO, MEMBER_NM, SEX_DIV, HP_NO, MEMBER_CARD_UID, QR_CD, FINGER1, FINGER2, DEL_YN, welfare_cd ) ' +
                ' VALUES ' +
                '( ' + QuotedStr(AMemberInfo.Code) + ', '
                     ...
                     + QuotedStr(sUse) +', '
                     + QuotedStr(AMemberInfo.WelfareCd) + ')';
...
function TLocalDB.DelMemberListToDB: Boolean;
var
  sSql: String;
begin
  Result := False;
